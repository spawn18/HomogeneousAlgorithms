...
def build_F(p, mu):
    def F(t):
        conditions = [(p[i-1][0] <= t) & (t < p[i][0]) for i in range(1, len(p))]
        funcs = [lambda x, i=j: np.max([p[i-1][1]-mu[2*(i-1)]*(x-p[i-1][0])**2/(p[i][0]-p[i-1][0]), p[i][1]-mu[2*(i-1)+1]*(p[i][0]-x)**2/(p[i][0]-p[i-1][0])], axis=0) for j in range(1, len(p))]
...
def min_F(p, mu):
    ...
    for i in range(1, len(p)):
        ...
        Q2 = mu[2*(i-1)+1]
        a = Q2 - Q1
        b = 2*Q1*p[i-1][0] - 2*Q2*p[i][0]
        c = (p[i-1][1]-p[i][1])*(p[i][0]-p[i-1][0]) - Q1*p[i-1][0]**2 + Q2*p[i][0]**2
        coefs = [a, b, c]
        roots = np.roots(coefs)
        roots = roots[np.logical_and(p[i-1][0] <= roots, roots <= p[i][0])]
        x.append(roots[0])

    r = [max([p[i-1][1]-mu[2*(i-1)]*((x[i-1]-p[i-1][0])**2)/(p[i][0]-p[i-1][0]), p[i][1]-mu[2*(i-1)+1]*((p[i][0]-x[i-1])**2)/(p[i][0]-p[i-1][0])]) for i in range(1, len(p))]
    p = list(zip(x,r))
...
def minimize(funcs, grad_smoother):
    ...
    for i, f in enumerate(funcs):
        ...
        if statistics.SAVE:
            ...
            plt.plot(xs, F(xs), 'limegreen', label='Criteria')
